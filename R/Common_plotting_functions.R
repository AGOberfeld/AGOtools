...
#' @title pShawe
#' @return img: the saved image , fName: the filename of the saved image
...
#' @author Thirsa Huisman
#' @title plot_basics_position_dodge
...
#' @import ggplot2
#' @export
plot_basics_position_dodge = function(error_bar_width = 1, point_size = 2, stroke_size = 1.5, line_width = 1, errorbar_line_width = 0.75, positiondodge_width = 0) {
...
#' @author Thirsa Huisman
#' @title plot_basics_position_jitter
#' @description This is a list of the basic plot elements that are used in most plots.
#' @details error_bar_width needs to be scaled according to the size of the x_axis, e.g., if x goes from 0 to 10 vs 0 to 1000, you need to scale the width accordingly.
#' @param error_bar_width width of the error bars
#' @param jitter_seed width of the error bars
#' @import ggplot2
#' @export
plot_basics_position_jitter = function(error_bar_width = 1, point_size = 2, stroke_size = 1.5, line_width = 1, errorbar_line_width = 0.75, jitter_width = 0, jitter_height=0, jitter_seed = 1) {
  plot_basics_list = list(stat_summary(fun = "mean", geom = "line", linewidth = line_width, position = position_jitter(width = jitter_width, height = jitter_height, seed = jitter_seed)),
                          stat_summary(fun.data = mean_se, geom = "errorbar", linewidth = errorbar_line_width, width = error_bar_width, linetype = "solid", position = position_jitter(width = jitter_width, height = jitter_height, seed = jitter_seed)),
                          stat_summary(fun = "mean", geom = "point", size = point_size, stroke = stroke_size, position = position_jitter(width = jitter_width, height = jitter_height, seed = jitter_seed)))
  return(plot_basics_list)
}
